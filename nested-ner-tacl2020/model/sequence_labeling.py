...
class BiRecurrentConvCRF4NestedNER(nn.Module):
    ...
    def predict(self, input_word_iv: Tensor, input_word_ooev: Tensor, input_char: Tensor, mask: Tensor, get_logits=False) \
        ...
        # Can I assume to np.mean over all CRFs? 

        all_CRF_energies = [crf.decode(output, mask=mask)[1] for crf in self.all_crfs]
        energy_batched = torch.mean(torch.stack(all_CRF_energies), 0)
