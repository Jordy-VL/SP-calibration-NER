...
public class IELinearBuilder_CONLL2003 {
	...
	public IELinearBuilder_CONLL2003(IELinearFeatureManager_CONLL2003 fm, SemanticTag[] tags){
		this._fm = fm;
		this._tags = tags;
	...
	public IELinearNetwork_CONLL2003 build(IELinearInstance inst){
		...
		if(!this._isLabeled)
		{
			...
			if(mode_a){
				...
				IELinearNetwork_CONLL2003 network = new IELinearNetwork_CONLL2003(inst, this._fm, this._nodes, this._children, this.getRootPositionForLen(inst.length())+1);
				return network;
			}
			else {
...
////			System.exit(1);
	...
	public IELinearNetwork_CONLL2003 build_labeled(IELinearInstance inst){
		IELinearNetwork_CONLL2003 network = new IELinearNetwork_CONLL2003(inst, this._fm);
...
//		System.err.println("There are "+network.countNodes()+" nodes. maximum: "+size);
	...
	public IELinearNetwork_CONLL2003 build_unlabeled(IELinearInstance inst){
		this._N = inst.length();
		IELinearNetwork_CONLL2003 network = new IELinearNetwork_CONLL2003(inst, this._fm);
	...
	public void build_unlabeled(){
		IELinearNetwork_CONLL2003 network = new IELinearNetwork_CONLL2003(null, null);
		this.build(network, -1);
		network.finalizeNetwork();
...
//		System.err.println(":"+NetworkIDMapper.viewHybridNode_ie(this._nodes[this._nodes.length-1], this._N, this._M));
	...
	public void build_labeled(IELinearNetwork_CONLL2003 network, IELinearInstance inst){
...
//						System.err.println("connected.."+(N+bIndex));
	...
	public void build(IELinearNetwork_CONLL2003 network, int bIndex){
