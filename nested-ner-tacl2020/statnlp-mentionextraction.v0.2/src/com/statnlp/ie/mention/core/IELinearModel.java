...
public class IELinearModel {
	...
	public void train(ArrayList<IELinearInstance> labeledInstances, int max_num_iterations, IEManager manager, NetworkParam param, String filename_model){
		...
		this.touch();
		this.train(max_num_iterations, manager, param, filename_model);
...
//			}
	...
	private void train(int max_num_iterations, IEManager manager, NetworkParam param, String filename_model){
		int nIterToSave = Integer.parseInt(System.getProperty("nIterToSave", "0"));
		boolean saveEveryNIters = nIterToSave > 0;
		for(int it = 0; it<max_num_iterations; it++){
			if(this.train_oneIteration(it))
				break;

			if(saveEveryNIters && it % nIterToSave == (nIterToSave-1) && it != max_num_iterations-1){
				System.err.println("Saving Model...");
				ObjectOutputStream out;
				try {
					out = new ObjectOutputStream(new FileOutputStream(filename_model+"-"+(it+1)));
