		...
		for(int k = 0; k<segs.size(); k++){
			...
			if(this._map_supported.containsKey(seg)){
				...
				for(Mention mention : mentions){
					...
					sb.append(seg.getBIndex()+","+seg.getEIndex()+","+mention.getHeadSegment().getBIndex()+","+mention.getHeadSegment().getEIndex()+" "+mention.getTag().getName());
				}
			}
		...
		for(int k = 0; k<this.length(); k++){
			...
			for(int i = 0; i<segs.size(); i++){
				...
				if(k==ts.getSegment().getBIndex()){
					if(ts.length()==1){
						tags.add("U-"+ts.getTag().getName());
					} else {
						tags.add("B-"+ts.getTag().getName());
					}
				} else if(k>ts.getSegment().getBIndex() && k<ts.getSegment().getEIndex()){
					tags.add("I-"+ts.getTag().getName());
				}
			}
	...
	public String toCRFPP_format_withType_BLIOU(){
		...
		for(int k = 0; k<this.length(); k++){
			...
			for(int i = 0; i<segs.size(); i++){
				...
				if(k==ts.getSegment().getBIndex()){
					if(ts.length()==1){
						sb.append("U-"+ts.getTag().getName());
						partOfSeg = true;
						break;
					} else {
						sb.append("B-"+ts.getTag().getName());
						partOfSeg = true;
						break;
					}
				} else if(k==ts.getSegment().getEIndex()-1){
					sb.append("L-"+ts.getTag().getName());
					partOfSeg = true;
					break;
				} else if(k > ts.getSegment().getBIndex() && k<ts.getSegment().getEIndex()-1){
					sb.append("I-"+ts.getTag().getName());
					partOfSeg = true;
					break;
				}
			}
...
//					if(tag_index%2==0)
					{
						if(k==seg.getBIndex()){
							if(mention.length()==1){
								sb.append("U-"+mention.getTag().getName());
								partOfSeg = true;
							} else {
								sb.append("B-"+mention.getTag().getName());
								partOfSeg = true;
							}
						} else if(k==seg.getEIndex()-1){
							sb.append("L-"+mention.getTag().getName());
							partOfSeg = true;
						} else if(k > seg.getBIndex() && k<seg.getEIndex()-1){
							sb.append("I-"+mention.getTag().getName());
							partOfSeg = true;
						}
					}
...
//					if(tag_index%2==1 && j == tag_index-1)
					if(j!=(tag_index+1)/2-1 || tag_index%2==0)
					{
						if(k==seg_head.getBIndex()){
							if(mention.length()==1){
								sb.append("\t");
								sb.append("HU-"+mention.getTag().getName());
								partOfSeg_head = true;
							} else {
								sb.append("\t");
								sb.append("HB-"+mention.getTag().getName());
								partOfSeg_head = true;
							}
						} else if(k==seg_head.getEIndex()-1){
							sb.append("\t");
							sb.append("HL-"+mention.getTag().getName());
							partOfSeg_head = true;
						} else if(k > seg_head.getBIndex() && k<seg_head.getEIndex()-1){
							sb.append("\t");
							sb.append("HI-"+mention.getTag().getName());
							partOfSeg_head = true;
						}
					}
